...
for a in range(0,len(ograte)):
	...
	if len(numbers) == 1:
		print("oxygen generator rating is", numbers2)
		c02rate = numbers
		break
	...
	for c in numbers:
		if int(c[a]) != ograte[a]: #if the index doesn't maych, put in bad list
	...
	print("numbers that should be removed:", numbersn)
	print("numbers that  should  be  kept:   ", numbersy)
...
for a in range(0,len(c02rate)):
	...
	print("-----------------------")
	print(f"orginial numbers left: {numbers2}")
	#print(f"loop {a} numbers2: {numbersy}")
