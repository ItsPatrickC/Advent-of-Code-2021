...
part2:
Built on part 1 and still returns part 1s answer too
loop through boards, when a winner is found, add it to winners list, 
when winners list length is at 100, break loop and show first and last winner upon their winning state
		'''
...
for a in numbers:
	...
	print(f"{len(winners)} have been bingoed")
	...
	for b in boards:
		...
		if bingocheck(b):
			...
			if len(winners) == 1:
				#print("first winner found")
				print(b)

		
	

print(f"first winner: {winners[0]} \n last winner: {winners[-1]}")
